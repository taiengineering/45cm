...
export async function debugRedis(): Promise<{ok:boolean; latency_ms?:number; error?:string}> {
  ...
  try {
    const pong = await Promise.race([conn().ping(), new Promise<never>((_,rej) => setTimeout(() => rej(new Error('ping timeout 5s')), 5000))]);
    return { ok: pong === 'PONG', latency_ms: Date.now() - start };
  } catch (e: any) { return { ok: false, error: e.message, latency_ms: Date.now() - start }; }
}
